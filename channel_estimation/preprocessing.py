...
from processChannelData import processChannelData
from processChannelRandomGeo import processChannelRandomGeo
...
def checkDataDirectory(path):
    ...
    if (path.joinpath('channel_data')).exists():
        print('Folder already exists')
    else:
        print('Creating the folder structure...\n')
        path.joinpath('channel_data').mkdir()
